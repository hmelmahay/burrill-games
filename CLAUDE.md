# CLAUDE.md — The Burr & Spill (burrill-games)
...
Local dev needs `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` in `.env.local` (both are public browser-side values). For a build-only check, any placeholder works.
